...
int FacebookApi::getVideoViews(String videoID){
	String command = "/v8.0/"+ videoID + "/video_insights/total_video_views/lifetime?access_token="+ _accessToken;
	//command += videoID;
	//command += "/video_insights/total_video_views/lifetime?access_token="+ _accessToken;
	String response = sendGetToFacebook(command);
	DynamicJsonDocument doc(1024);
	DeserializationError error = deserializeJson(doc, response);
	if(!error){
		if(doc.containsKey("data")){
			return doc["data"][0]["values"][0]["value"];
		}else{
			Serial.println("JSON response was not as expected");
			Serial.println(response);
		}
	}else{
		Serial.println("Failed to parse JSON. Error:");
